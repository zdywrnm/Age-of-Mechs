...
export const QUESTS = [
  ...
  },
  {
    ...
    progress: (c, ctx) => {
      ...
      return nearHenge ? (py < ctx.hengeGround - 2 ? `还差 ${remain} 格遇到第一块木板！` : '从巨石阵正中间往下挖！') : '去西北高地的巨石阵'
    },
  },
  {
    ...
    lines: [
      ...
      '你已经很强了，去试试真正的挑战吧！',
      '城西海上小岛的刷怪塔，从一级怪物一直排到最强 boss——一共有一千层！出西门过石桥就到！',
      '塔里有我做的传送台，站上去选楼层就行。先打到第 10 层，会会小 boss 黑蛛王！',
    ],
    progress: c => c.floor > 0 ? `当前第 ${c.floor} 层 → 目标第 10 层` : '出西门过石桥去刷怪塔小岛，站上大厅的金色传送台',
  },
  {
  ...
  },
  {
    ...
    lines: [
      '接下来是【暗黑地狱】！传送门就在主岛东南海岸的沙滩上。',
      '开门方法：往传送门顶部放一块金子，门就会自动开启！',
      '注意：无论放多少金子都会全部消耗，而且每次开门只够一人进入返回一次！',
      '地狱里岩浆湖圈的正中央有一棵大树——森林变形齿轮就在树顶！走岩浆石，别碰岩浆！',
    ],
    progress: (c, ctx) => ctx.player.hasGear('forest') ? '拿到了！' : '带上金子去东南海岸的大门（挖金矿或用背包里的金子）',
  },
  {
  ...
  },
  {
    ...
    lines: [
      ...
      '入口就在城里的广场上——石砖框的井口。跳下去有水垫接着，按 E 也能直达！',
      '城里有驱动核心——它驱动着整个世界，地下族人世世代代守护着它。',
      '光明变形齿轮就在驱动核心边缘的箱子里。顺便替我看看第三座神殿！',
    ],
    progress: (c, ctx) => ctx.player.hasGear('light') ? '拿到了！' : '入口在城内广场（石砖框井口），下去找驱动核心',
  },
  {
